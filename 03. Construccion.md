...
Cambiar el usuario de Docker Hub

```
docker build . -t <DOCKER_USER>/rest_api:1.0.0 --build-arg APP_ENV="$APP_ENV_KUBE_API"

docker login

docker run --rm --name rest-api -p 3000:3000 <DOCKER_USER>/rest_api:1.0.0
...
```
docker push <DOCKER_USER>/rest_api:1.0.0
```
