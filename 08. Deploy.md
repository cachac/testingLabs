...
```
kubectl exec -it $(kubectl get pods  -l app=loadtester  -o jsonpath='{.items[*].metadata.name}') -- hey -z 120s -q 100 -c 20 http://carlos.kubelabs.dev/error

kubectl rollout restart deploy demo-frontend
```
Resultado:
```
Normal   Synced  3m    flagger  Halt 	demo-fronted.demo advancement success rate 69.17% < 99%
```
